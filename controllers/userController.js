...
const verify = async (req, res) => {
    try {
        ...
        const updateverify= userModel.findByIdAndUpdate(id,{isVerified:true}, {new: true})
    
        //const {username, Lastname, email} = user
        if(updateverify.isVerified==true){
        return  res.status(201).json(`Congratulations ${user.username}, you have been verified`)
             ...
             jwt.verify(user.token, process.env.secret,(error)=>{
                ...
                if(error){
                  ...
                  const token  = jwt.sign({ username,email }, process.env.secret, { expiresIn: "5min" })
                    user.token=token
                    user.save()
                    const link=`${req.protocol}://${req.get("host")}/verify/${user.id}/${user.token}`
                    sendMail({
...
const deleteUser = async (req, res) => {
    try{
      ...
      if (!user){
          res.status(404).json({
            message: `User with id: ${userId} is not found.`});return
      }
      // delete the user
      await userModel.findByIdAndDelete(user)
      return res.status(200).json({
          message: `User with the name ${user.username} was successfully deleted.`,
          //data: user,
      }); 
  }catch (err) {
      res.status(500).json({ 
...
module.exports= {signUp, verify, logIn, getOne, getAll, deleteUser, signOut, forgotPassword, resetPassword }